...
import PortalNegociacaoBlock from "@/components/PortalNegociacaoBlock";
import InsightsBlock from "@/components/InsightsBlock";
import { JC_CONFIG, JC_BLOCKS, BLOCK_NAMES } from "@/lib/constants";

export default function JornadaCobrancaPage() {
  ...
  useEffect(() => {
    const saved = getTriagemData(JC_CONFIG.storageKey);
  ...
  const removeBlock = (id: string) => { const c = JC_BLOCKS[id] || []; setHiddenBlocks(p => new Set([...p, id])); setHiddenServices(p => new Set([...p, ...c])); };
  ...
  return (
    <>
      {showTriagem && <TriagemWizard config={JC_CONFIG} onStart={(d) => { setTriagemData(d); setShowTriagem(false); }} onSkip={() => setShowTriagem(false)} />}
      <div className="bg-[var(--color-navy-05)] min-h-dvh w-full overflow-x-hidden p-1 sm:p-2">
        <div id="page-wrapper" className="w-full bg-white rounded-[28px] border-2 border-[var(--color-navy-20)] p-3 sm:p-7 sm:pt-6 relative flex flex-col overflow-hidden">
          <ActionButtons config={JC_CONFIG} blockServices={JC_BLOCKS} blockNames={BLOCK_NAMES}
            ...
            currentSlug="jornada-cobranca"
          />
          <Header title="Jornada de" strong="Cobrança" />
          <div className="flex-1 flex flex-col min-h-0">
            <div className="main-layout-row-parent service-layout-row flex flex-col xl:flex-row flex-wrap gap-2 sm:gap-3 items-stretch flex-1 min-h-0">
              ...
              <div className="connector-dot service-flex-block"><PGChannelsBlock hidden={isHidden("block-channels")} hiddenServices={hiddenServices} serviceIds={JC_BLOCKS["block-channels"]} onRemoveBlock={() => removeBlock("block-channels")} onRemoveService={toggleService} onRestoreService={toggleService} showCarta={false} showAds={false} /></div>
              <div className="connector-dot service-flex-block"><PGFilesBlock hidden={isHidden("block-files")} hiddenServices={hiddenServices} serviceIds={JC_BLOCKS["block-files"]} onRemoveBlock={() => removeBlock("block-files")} onRemoveService={toggleService} onRestoreService={toggleService} compact /></div>
              ...
              <div className="connector-dot service-flex-block"><AtendimentoBlock hidden={isHidden("block-atendimento")} hiddenServices={hiddenServices} serviceIds={JC_BLOCKS["block-atendimento"]} onRemoveBlock={() => removeBlock("block-atendimento")} onRemoveService={toggleService} onRestoreService={toggleService} compact /></div>
              <div className="connector-dot service-flex-block"><PortalNegociacaoBlock hidden={isHidden("block-portal-ga")} onRemove={() => removeBlock("block-portal-ga")} triagemData={triagemData} /></div>
            </div>
          </div>
          <div className="mt-2 sm:mt-[10px] flex-shrink-0 bg-white rounded-[26px] border-2 border-[var(--color-navy-20)]">
            <InsightsBlock hidden={isHidden("block-insights")} hiddenServices={hiddenServices} serviceIds={JC_BLOCKS["block-insights"]} onRemoveBlock={() => removeBlock("block-insights")} onRemoveService={toggleService} onRestoreService={toggleService} />
          </div>
        </div>
      </div>
    </>
