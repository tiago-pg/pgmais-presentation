        ...
        <div id="page-wrapper" className="w-full bg-white rounded-[28px] border-2 border-[var(--color-navy-20)] p-3 sm:p-7 sm:pt-6 relative flex flex-col overflow-hidden">
          ...
          <div className="flex-1 flex flex-col min-h-0">
            <div className="flex flex-col lg:flex-row flex-wrap lg:flex-nowrap gap-2 sm:gap-3 items-stretch flex-1 min-h-0">
              ...
              <PGChannelsBlock hidden={hiddenBlocks.has("block-channels")} hiddenServices={hiddenServices} serviceIds={GA_BLOCKS["block-channels"]} onRemoveBlock={() => removeBlock("block-channels")} onRemoveService={toggleService} onRestoreService={toggleService} showCarta={false} showAds={false} />
              <PersonArea triagemData={triagemData} showBadge badgeText="INTERAÇÃO CLIENTE" badgeSub="Texto ou voz" />
              <AtendimentoBlock hidden={hiddenBlocks.has("block-atendimento")} hiddenServices={hiddenServices} serviceIds={GA_BLOCKS["block-atendimento"]} onRemoveBlock={() => removeBlock("block-atendimento")} onRemoveService={toggleService} onRestoreService={toggleService} />
              <PortalAutoBlock hidden={hiddenBlocks.has("block-portal-ga")} onRemove={() => removeBlock("block-portal-ga")} triagemData={triagemData} />
            </div>
          </div>
          <div className="mt-2 sm:mt-[10px] flex-shrink-0 bg-white rounded-[26px] border-2 border-[var(--color-navy-20)]">
            <InsightsBlock hidden={hiddenBlocks.has("block-insights")} hiddenServices={hiddenServices} serviceIds={GA_BLOCKS["block-insights"]} onRemoveBlock={() => removeBlock("block-insights")} onRemoveService={toggleService} onRestoreService={toggleService} />
          </div>
        </div>
      </div>
    </>
